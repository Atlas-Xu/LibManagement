package cn.xchub.web.face.factory;
...
public class FaceEngineFactory extends BasePooledObjectFactory<FaceEngine> {
    ...
    public FaceEngine create() {
        ...
        if (activeCode != ErrorInfo.MOK.getValue() && activeCode != ErrorInfo.MERR_ASF_ALREADY_ACTIVATED.getValue()) {
            ...
            throw new BusinessException(BusinessExceptionEnum.FAIL.getCode(), "引擎激活失败" + activeCode);
        }
        int initCode = faceEngine.init(engineConfiguration);
        if (initCode != ErrorInfo.MOK.getValue()) {
            log.error("引擎初始化失败" + initCode);
            throw new BusinessException(BusinessExceptionEnum.FAIL.getCode(), "引擎初始化失败" + initCode);
        }
        return faceEngine;
    }
